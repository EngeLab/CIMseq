...
2. Dimensionality reduction with the singlets. Here we perform dimensionality reduction with the single cells using tSNE and classify the number of clusters, to deduce the possible cell types present in the tissue.
    + Feature eslection.
    + Distance calculation.
    + t-SNE
    + Classification.
3. Multuplet deconvolution via swarm optimization. Here we utilize swarm optimization to deconvolute the multuplets and, thus, provide an overview of the tissue "connectome".
...
```{r}
table(colnames(getData(cObjSng, "counts")))
```
...
spPlot(cObjSng, cObjMul, type="ercc")
```
...
```{r, fig.align='center', fig.height=8, fig.width=10, eval=TRUE, message=FALSE}
spPlot(cObjSng, cObjMul, type="markers", markers=c("c1", "b1"))
```
...
```{r, fig.align='center', fig.height=8, fig.width=10, eval=TRUE, message=FALSE}
spPlot(uObj, cObjSng, type="markers", markers=c("c1", "b1"))
```
...
```{r}
sObj <- spSwarm(cObjMul, uObj, swarmsize = 150, cores=2, maxiter=10)
```

### spSwarm results

We can exmaine the resulting sonnections with the `spSwarmPoisson` command:

```{r}
head(spSwarmPoisson(sObj, edge.cutoff=1/10.5, min.num.edges=0, min.pval=1))
```
...
```{r, fig.align='center', fig.height=8, fig.width=10, eval=TRUE, message=FALSE}
spPlot(sObj, uObj, cObjSng, edge.cutoff=1/10.5, min.num.edges=1)
```

The network plot can be made in the above way or with a "natural" layout as below:

```{r, fig.align='center', fig.height=8, fig.width=10, eval=TRUE, message=FALSE, warning=FALSE}
spPlot(sObj, uObj, cObjSng, layout="igraph", edge.cutoff=1/10.5, min.num.edges=1)
```
