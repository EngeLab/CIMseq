...
author: "Jason T. Serviss and Martin "
date: '`r Sys.Date()`'
output:
    html_document:
        theme: flatly
...
1. Creating the sp.scRNAseq counts object. 
    Here the raw count data, ERCC spike-in count data, and the information indicating which samples are singlets and which are multuplets is entered.
2. Unsupervised clustering of the singlets. 
    Here we perform unsupervised clustering of the singlets using tSNE and classify the number of clusters, to deduce the possible cell types in the multuplets.
3. Multuplet deconvolution via swarm optimization.
    Here we utilize swarm optimization to deconvolute the multuplets and, thus, provide a picture of the tissue "connectome".

## sp.scRNAseq counts object
...
We use the testData and testErcc variables to initialize the sp.scRNAseq counts object. The testData variable contains singlets that comprise 10 cell types. Each cell type has 85 singlets and each singlet has 250 gene expression values. The dataset also includes 2 multuplets comprised of 4 different cell types each. The multuplets are comprised in a fashon that, in the final results, there should be one connection between all cell types with the exception of cell type E1 and F1. See below:

```{r}
testData[1:2, 1:2] #example
dim(testData) #genes and sample numbers
table(colnames(testData)) # sample names
head(rownames(testData)) #example of gene names
class(testData) #must be a matrix
```
...
## sp.scRNAseq unsupervised clustering object
