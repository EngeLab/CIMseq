...
```{r}
#make one spCounts object for singlets and one for multuplets
sng <- grepl("s.", colnames(testCounts)) #identify singlets
mul <- grepl("m.", colnames(testCounts)) #identify multuplets

sngCounts <- testCounts[ ,sng] #counts for singlets
mulCounts <- testCounts[ ,mul] #counts for multuplets

sngErcc <- testErcc[ ,sng] #ERCC for singlets
mulErcc <- testErcc[ ,mul] #ERCC for multuplets

cObj.sng <- spCounts(sngCounts, sngErcc) #singlet spCounts object
cObj.mul <- spCounts(mulCounts, mulErcc) #multuplet spCounts object

cObj.sng #show singlet counts object
```
...
```{r}
colnames(getData(cObj.mul, "counts"))
```
...
plot(cObj.sng, cObj.mul, type="ercc")
```
...
```{r, fig.align='center', fig.height=8, fig.width=10, eval=TRUE, message=FALSE}
plot(cObj.sng, cObj.mul, type="markers", markers=c("c1", "b1"))
```
