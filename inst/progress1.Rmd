...
make.synthetic.slice <- function(cell.types, fractions) {
    fractions <- fractions/sum(fractions) #this makes sure that the sum of the fractions is always 1
    cat(fractions, "\n\n")
    res <- apply(cell.types, 1, function(x) {sum(x*fractions)}) #this gives the value for each gene in the multuplet dependant on the fractions variable
    return(res)
}

dist.to.slice <- function(fractions, cell.types, slice) {
    #o <- sort(fractions, decreasing=T)
    min <- 1/length(fractions)
    fractions[fractions < min] <- min
    #fractions[-1:-5] <- 0 # Consider only top 5 fractions
...
l <- list(
    dbl.top2000[ ,type==unique(type)[1]],
    dbl.top2000[ ,type==unique(type)[2]],
    dbl.top2000[ ,type==unique(type)[3]],
    dbl.top2000[ ,type==unique(type)[4]]
)
names(l) <- unique(type)
set.seed(3)
samp <- lapply(l, function(x) x[,sample(1:ncol(x), replace=FALSE, size=5)])
dbl.top2000 <- matrix(c(unlist(samp)), ncol=length(samp)*5)
colnames(dbl.top2000) <- unlist(lapply(samp, function(x) colnames(x)))
...
##analyze results
groups.mat <- data.frame(lapply(optim.res.all.2, function(x) {unlist(x[1:3])}))
colnames(groups.mat) <- colnames(dbl.top2000)
groups.mat[is.na(groups.mat) == TRUE] <- 0
heatmap(t(groups.mat))
```
...
library(nloptr)
#, lower=0, upper=1.0
slsqp <- lapply(1:(dim(dbl.top2000)[2]), function(i) {
    slsqp(x0=fractions, fn=dist.to.slice, gr=NULL, cell.types=clusterMeans.top2000, slice=dbl.top2000[,i])
})
groups.mat2 <- data.frame(lapply(slsqp, function(x) {unlist(x[1:3])}))[1:3, ]
colnames(groups.mat2) <- colnames(dbl.top2000)
heatmap(t(groups.mat2))
```

```{r}
library(pso)
pso <- lapply(1:(dim(dbl.top2000)[2]), function(i) {
    psoptim(par = fractions, fn = dist.to.slice, cell.types=clusterMeans.top2000, slice=dbl.top2000[,i], lower=0, upper=1)
})
groups.mat3 <- as.data.frame(lapply(pso, function(x) x$par))
colnames(groups.mat3) <- colnames(dbl.top2000)
apply(groups.mat3, 2, function(x) x/sum(x))
```
