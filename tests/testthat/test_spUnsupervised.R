...
test_that("check that spTopVar outputs the expected result", {
    ...
    #run function
    output <- spTopVar(test_spCountsSng, 10)
...
test_that("check that spTopMax outputs the expected result", {
    ...
    #run function
    output <- spTopMax(test_spCountsSng, 10)
...
test_that("check that pearsonsDist outputs the expected result", {
  ...
  #setup expected data
  expectHead <- c(0, 0, 0, 0, 0, 1)
  expectTail <- c(0, 0, 0, 0, 0, 0)
    
  #run function
  output <- pearsonsDist(test_spCountsSng, select)
...
test_that("check that runTsne outputs the expected result", {
  ...
  #run function
  output <- runTsne(
    my.dist, dims, theta, initial_dims,
    max_iter, perplexity, seed, is_distance
  )
...
test_that("check that tsneGroupMeans outputs the expected result",{
  ...
  tsne <- getData(test_spUnsupervised, "tsne")
...
test_that("check that erccPerClass outputs the expected result",{
  ...
  expected1 <- tibble::tibble(
    class = c("A375", "HCT116", "HOS"),
    medianFracErcc = rep(0, 3),
    meanFracErcc = c(0.006117311, 0.010156366, 0.006337655)
  )
    
  #run function
  output <- erccPerClass(
    test_spCountsSng, test_spCountsMul, test_spUnsupervised
  )
